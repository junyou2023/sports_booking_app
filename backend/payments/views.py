...
class StripeCheckoutView(APIView):
    ...
    def post(self, request):
        ...
        # slot must not belong to the requesting user
        if (
            slot.activity.organization
            and slot.activity.organization.members.filter(user=request.user).exists()
        ):
            return Response({"detail": "cannot book your own slot"}, status=400)

        # slot must not have started or be in the past
        if slot.begins_at <= timezone.now():
            return Response({"detail": "slot already started"}, status=400)
        ...
        booking = Booking.objects.filter(
            slot=slot, user=request.user, status=Booking.STATUS_PENDING
        ).first()
        if not booking:
